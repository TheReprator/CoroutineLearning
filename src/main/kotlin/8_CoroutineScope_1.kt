...
*
* First we extend the class with CoroutineScope(line 8), then we override the only property coroutineContext(line 14)
* Since, Dispatchers.Main is considered the best option as the default dispatcher for android,
*
*
* Even better, it is a common practice to not cancel the whole scope but only its children(line 20 and 21 are equivalent).
...
* we should use SupervisorJob(line 14) instead of Job.
*
* The last important functionality is the default way of handling un-aught exceptions. On Android, we often define what
* should happen in the case of different kinds of exceptions. So we had added CoroutineExceptionHandler(line 10) and
* made it as part of context (line 14), so that it can catch any uncaught exception
*
